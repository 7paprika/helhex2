...
if Turns_per_Tube > 0 and d_i > 0 and not lmtd_error:
    ...
    with tab_cs:
        ...
        fig_cs = create_cross_section_fig(
            shell_od, st.session_state['D_s'], st.session_state['D_mandrel'],
            st.session_state['D_c'], st.session_state['d_o'], d_i,
            N_p_val, st.session_state['shell_thick'], st.session_state['t_thick'])
        st.plotly_chart(fig_cs, use_container_width=True)

    # ========== TAB 2: 종단면도 ==========
    with tab_ls:
        st.caption("🔍 중심축을 관통하는 종단면 — Shell 길이, Pitch, 노즐 배치 (헬리컬 좌/우 피치 오프셋 반영)")
        fig_ls, _ = create_longitudinal_fig(
            st.session_state['D_s'], st.session_state['D_mandrel'], st.session_state['D_c'],
            st.session_state['d_o'], N_p_val, st.session_state['shell_thick'],
            st.session_state['pitch'], Shell_TT_Length_mm, L_shell_mm, Turns_per_Tube)
